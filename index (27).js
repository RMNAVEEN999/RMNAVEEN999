...
class AppStore extends Components {
  state = {
    searcInput: '',
  ...
  getActiveTabApps = searcApps => {
    const {activeTabId} = this.state
    const filterApps = searcApps.filter(
      eachSearchApp => eachSearchApp.category === activeTabId,
    )
    return filterApps
  }
  getSearchResult = () => {
    const {searcInput} = this.state
  ...
  render() {
    const {searcInput, activeTabId} = this.state
    const searchResult = this.getSearchResult()
    const filterApps = this.getActiveTabApps(searchResult)
